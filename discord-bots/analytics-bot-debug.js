...
async function loadTrackedChannels() {
  try {
    ...
    console.log(`[DEBUG] Found ${projectKeys.length} project keys`)
    
    for (const key of projectKeys) {
      const project = await redis.json.get(key)
      console.log(`[DEBUG] Project ${key}: active=${project?.isActive}, channels=${project?.trackedChannels?.length}`)
      ...
      if (project && project.isActive && project.trackedChannels?.length > 0) {
        ...
        console.log(`[DEBUG] Channel IDs:`, project.trackedChannels)
    ...
    console.log(`[DEBUG] Project cache:`, Array.from(projectChannelsCache.entries()).map(([k, v]) => ({ id: k, name: v.name, channels: Array.from(v.channels) })))
  } catch (error) {
...
async function saveMessage(message, projectId, projectData) {
  try {
    console.log(`[DEBUG] Saving message for project ${projectData.name}`)
    ...
    console.log(`[DEBUG] Message saved with ID: ${messageId}`)
...
async function updateProjectStats(projectId) {
  try {
    ...
    console.log(`[DEBUG] Updated project stats: ${messageIds.length} messages, ${userIds.length} users`)
  } catch (error) {
...
client.on('ready', async () => {
  ...
  // Debug: List all servers
  console.log('[DEBUG] Connected servers:')
...
client.on('messageCreate', async (message) => {
  ...
  console.log(`[DEBUG] Message from ${message.author.username} in ${message.guild?.name || 'DM'}#${message.channel?.name || 'DM'}: "${message.content.substring(0, 50)}${message.content.length > 50 ? '...' : ''}"`)
  ...
  console.log(`[DEBUG] Checking channel ${message.channel.id} against ${projectChannelsCache.size} projects`)
  
  for (const [projectId, data] of projectChannelsCache) {
    console.log(`[DEBUG] Checking project ${data.name}: channels=${Array.from(data.channels)}`)
    if (data.channels.has(message.channel.id)) {
      trackedProject = projectId
      projectData = data
      console.log(`[DEBUG] MATCH! Channel is tracked by project ${data.name}`)
      break
    }
  }
  ...
  if (!trackedProject) {
    console.log(`[DEBUG] Channel ${message.channel.name} (${message.channel.id}) is NOT tracked`)
    return
  }
  
  console.log(`[DEBUG] Processing message for project: ${projectData.name}`)
