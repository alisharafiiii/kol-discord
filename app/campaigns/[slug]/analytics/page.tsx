...
export default function CampaignAnalyticsPage({ params }: { params: { slug: string } }) {
  ...
  const exportAnalytics = () => {
    const data = {
      ...
      budgetAllocation: analytics.budgetByTier,
    }
    ...
    a.href = url
    a.download = `${campaign?.slug || 'campaign'}-analytics-${new Date().toISOString().split('T')[0]}.json`
      ...
      <div className="max-w-7xl mx-auto">
              ...
              {analytics.tierDistribution.map((tier) => (
                <div key={tier.name}>
                  <div className="flex justify-between mb-1">
                    <span className="text-sm text-green-300">{tier.name}</span>
                    <span className="text-sm text-green-400">{tier.value} KOLs</span>
                  </div>
                  <div className="w-full bg-black rounded-full h-6 relative overflow-hidden">
                    <div
                      className="h-full bg-green-600 rounded-full transition-all duration-500"
                      style={{ width: `${(tier.value / analytics.totalKOLs) * 100}%` }}
                    />
                    <span className="absolute inset-0 flex items-center justify-center text-xs text-green-100">
                      {((tier.value / analytics.totalKOLs) * 100).toFixed(0)}%
                    </span>
                  </div>
                  <div className="flex justify-between mt-1 text-xs text-green-500">
                    <span>{tier.views.toLocaleString()} views</span>
                    <span>${tier.budget.toLocaleString()}</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-3">
              {analytics.platformBreakdown.map((platform) => (
                <div key={platform.name}>
                  <div className="flex justify-between mb-1">
                    <span className="text-sm text-green-300">{platform.name}</span>
                    <span className="text-sm text-green-400">{platform.value} KOLs</span>
                  </div>
                  <div className="w-full bg-black rounded-full h-6 relative overflow-hidden">
                    <div
                      className="h-full bg-blue-600 rounded-full transition-all duration-500"
                      style={{ width: `${(platform.value / analytics.totalKOLs) * 100}%` }}
                    />
                    <span className="absolute inset-0 flex items-center justify-center text-xs text-blue-100">
                      {((platform.value / analytics.totalKOLs) * 100).toFixed(0)}%
                    </span>
                  </div>
        ...
        <div className="bg-green-900/20 border border-green-500 rounded-lg p-6 mb-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            {analytics.budgetByTier.map((tier) => (
              <div key={tier.name} className="text-center">
                <div className="text-2xl font-bold text-green-300">
                  ${(tier.value / 1000).toFixed(1)}K
                </div>
                <div className="text-sm text-green-400">{tier.name}</div>
                <div className="text-xs text-green-500">{tier.percentage.toFixed(1)}%</div>
              </div>
            ))}
          </div>
        </div>
